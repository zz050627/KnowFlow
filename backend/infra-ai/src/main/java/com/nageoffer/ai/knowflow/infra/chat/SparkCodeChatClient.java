...
package com.nageoffer.ai.knowflow.infra.chat;

import cn.hutool.core.collection.CollUtil;
import com.google.gson.Gson;
import com.google.gson.JsonArray;
import com.google.gson.JsonObject;
import com.nageoffer.ai.knowflow.framework.convention.ChatMessage;
import com.nageoffer.ai.knowflow.framework.convention.ChatRequest;
import com.nageoffer.ai.knowflow.framework.trace.RagTraceNode;
import com.nageoffer.ai.knowflow.infra.config.AIModelProperties;
import com.nageoffer.ai.knowflow.infra.enums.ModelCapability;
import com.nageoffer.ai.knowflow.infra.enums.ModelProvider;
import com.nageoffer.ai.knowflow.infra.http.HttpMediaTypes;
import com.nageoffer.ai.knowflow.infra.http.ModelClientErrorType;
import com.nageoffer.ai.knowflow.infra.http.ModelClientException;
import com.nageoffer.ai.knowflow.infra.http.ModelUrlResolver;
import com.nageoffer.ai.knowflow.infra.model.ModelTarget;
...
public class SparkCodeChatClient implements ChatClient {
    ...
    @Override
    @RagTraceNode(name = "sparkcode-chat", type = "LLM_PROVIDER")
    public String chat(ChatRequest request, ModelTarget target) {
        ...
        try (Response response = httpClient.newCall(requestHttp).execute()) {
            if (!response.isSuccessful()) {
                ...
                log.warn("SparkCode 同步请求失败: status={}, body={}", response.code(), body);
                throw new ModelClientException(
                        "SparkCode 同步请求失败: HTTP " + response.code(),
            ...
            throw new ModelClientException("SparkCode 同步请求失败: " + e.getMessage(),
    ...
    @Override
    @RagTraceNode(name = "sparkcode-stream-chat", type = "LLM_PROVIDER")
    ...
    private void doStream(Call call, StreamCallback callback, AtomicBoolean cancelled) {
        try (Response response = call.execute()) {
            if (!response.isSuccessful()) {
                String body = readBody(response.body());
                throw new ModelClientException(
                        "SparkCode 流式请求失败: HTTP " + response.code() + " - " + body,
                        classifyStatus(response.code()),
                        response.code()
                );
            }
            ...
            if (body == null) {
                throw new ModelClientException("SparkCode 流式响应为空", ModelClientErrorType.INVALID_RESPONSE, null);
            }
            ...
            while (!cancelled.get()) {
                ...
                try {
                    ...
                    log.warn("SparkCode 流式响应解析失败: line={}", line, parseEx);
                }
            }
            if (!cancelled.get() && !completed) {
                throw new ModelClientException("SparkCode 流式响应异常结束", ModelClientErrorType.INVALID_RESPONSE, null);
            }
        } catch (Exception e) {
            callback.onError(e);
        }
    ...
    private String requireModel(ModelTarget target) {
        if (target == null || target.candidate() == null || target.candidate().getModel() == null) {
            throw new IllegalStateException("SparkCode 模型名称缺失");
        }
    ...
    private JsonObject parseJsonBody(ResponseBody body) throws IOException {
        if (body == null) {
            throw new ModelClientException("SparkCode 响应为空", ModelClientErrorType.INVALID_RESPONSE, null);
        }
    ...
    private String extractChatContent(JsonObject respJson) {
        if (respJson == null || !respJson.has("choices")) {
            throw new ModelClientException("SparkCode 响应缺少 choices", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        JsonArray choices = respJson.getAsJsonArray("choices");
        if (choices == null || choices.isEmpty()) {
            throw new ModelClientException("SparkCode 响应 choices 为空", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        JsonObject choice0 = choices.get(0).getAsJsonObject();
        if (choice0 == null || !choice0.has("message")) {
            throw new ModelClientException("SparkCode 响应缺少 message", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        JsonObject message = choice0.getAsJsonObject("message");
        if (message == null || !message.has("content") || message.get("content").isJsonNull()) {
            throw new ModelClientException("SparkCode 响应缺少 content", ModelClientErrorType.INVALID_RESPONSE, null);
        }
